...
class ProductService:
    ...
    def _sync_card_value_with_products(self, card_id: int) -> None:
        """
        Sincroniza o valor do card com o total de produtos + frete - desconto global.
        ...
        """
        totals = self.repository.get_card_products_total(card_id)
        total_products = totals["total"]  # já inclui descontos por produto
        ...
        if card:
            ...
            if card.payment_info and isinstance(card.payment_info, dict):
                gd = card.payment_info.get("global_discount") or 0
                gdt = card.payment_info.get("global_discount_type", "value")
            ...
            card.value = total_products - global_discount_amount + shipping
            self.db.commit()
