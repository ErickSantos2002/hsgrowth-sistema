"""
Script de correção: move cards perdidos do board Prospecção para a lista correta.

Situação: 153 cards com is_won=-1 (perdido) no board Prospecção (board_id=6)
estão em listas de pipeline em vez da lista 'Negócio Perdido' (list_id=27).

Execução:
    # Dry-run (só mostra o que seria feito, sem alterar nada):
    python scripts/fix_lost_cards_prospeccao.py

    # Execução real:
    python scripts/fix_lost_cards_prospeccao.py --apply
"""
import sys
import os
...
# ─── Configuração ────────────────────────────────────────────────────────────
BOARD_ID = 6           # Prospecção
TARGET_LIST_ID = 27    # Negócio Perdido (is_lost_stage=True)
...
def run():
    ...
    try:
        # Busca cards perdidos no board 6 fora da lista 27
        cards = (
            db.query(Card)
        ...
        print(f"\n{'=' * 60}")
        print(f"  Correção: cards perdidos fora da lista Negócio Perdido")
        print(f"  Board: Prospecção (id={BOARD_ID})")
        print(f"  Lista destino: Negócio Perdido (id={TARGET_LIST_ID})")
        print(f"  Modo: {mode}")
        ...
        if DRY_RUN:
            ...
            for card in cards[:20]:
                ...
                print(f"  #{card.id} | {card.title[:55]!r} | {from_name} → Negócio Perdido")
            if len(cards) > 20:
                print(f"  ... e mais {len(cards) - 20} cards")
            print(f"\nPara aplicar, rode: python scripts/fix_lost_cards_prospeccao.py --apply\n")
            return
        ...
        for card in cards:
            try:
                ...
                db.commit()
                moved += 1
                print(f"  [OK] #{card.id} | {card.title[:50]!r} | {from_name} → Negócio Perdido")
