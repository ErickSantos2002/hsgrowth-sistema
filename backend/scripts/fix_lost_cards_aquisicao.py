"""
Script de correção: move cards perdidos do board Aquisição para a lista correta.

Situação: cards com is_won=-1 (perdido) no board Aquisição (board_id=7)
estão em listas de pipeline em vez da lista 'Negócio Perdido' (list_id=33).

Execução:
    # Dry-run (só mostra o que seria feito, sem alterar nada):
    python scripts/fix_lost_cards_aquisicao.py

    # Execução real:
    python scripts/fix_lost_cards_aquisicao.py --apply
"""
import sys
import os
...
TARGET_LIST_ID = 33    # Negócio Perdido (is_lost_stage=True)
...
def run():
    ...
    try:
        # Busca cards perdidos no board 7 fora da lista 33
        cards = (
            db.query(Card)
        ...
        if DRY_RUN:
            ...
            print(f"\nPara aplicar, rode: python scripts/fix_lost_cards_aquisicao.py --apply\n")
            return
