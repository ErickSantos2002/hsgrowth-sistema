"""
Transportadoras — Lote 14 — 200 leads: 100 para cada (Miguel e Karolaine)
  - Miguel Luiz Pereira de Melo         (id=16)  — VENDEDOR
  - Karolaine Martins                    (id=9)   — VENDEDOR

Fonte: Planilha_Transportadoras_Prospeccao_fixed.xlsx (padronizada de Planilha_Nova_030726).

PRIMEIRO lote com Miguel/Karol como VENDEDORES: eles são atribuídos na coluna
`Vendedor_Responsavel` (-> card.assigned_to_id), não em SDR. Claudia continua
sendo SDR (coluna `SDR_Responsavel *`). Roteamento por nome via VENDEDORES.
...
O que faz:
  ...
  - Pega as próximas 200 linhas SEM Status_Importacao
  - Distribui 100 por pessoa na ordem acima, cada um na sua coluna (SDR ou Vendedor)
  - Preenche Canal_Aquisicao = "Outbound" / Detalhe = "Outbound - Lista fria"
  - Marca como "Importado" na coluna Status_Importacao
  - Salva transportadoras_lote14.xlsx para passar ao import_from_planilha.py
"""
...
OUTPUT_LOTE = os.path.join(SCRIPT_DIR, "Planilha_Transportadoras_Prospeccao_lote14.xlsx")
...
# Miguel e Karolaine agora são VENDEDORES -> coluna Vendedor_Responsavel (assigned_to_id).
# Qualquer nome fora deste conjunto vai para SDR_Responsavel (ex.: Claudia).
...
def main():
    print("=" * 60)
    print("TRANSPORTADORAS LOTE 14 — 200 cards (Miguel 100 + Karolaine 100 — VENDEDORES)")
    print("=" * 60)
    ...
    # Sequência de nomes conforme as quantidades
    assignment = []
    for nome, n in PESSOAS:
        assignment.extend([nome] * n)
    assignment = assignment[:len(rows_to_process)]

    # Atribui cada linha na coluna certa (SDR ou Vendedor)
    ...
    print(f"\ntransportadoras_lote14.xlsx salvo com {len(rows_to_process)} linhas (dados a partir da row 5 — sem card faltante).")
    print("\nPróximo passo:")
    print("  python scripts/imports/import_from_planilha.py C:/Users/HS/Documents/GitHub/hsgrowth-sistema/backend/scripts/imports/Planilha_Transportadoras_Prospeccao_lote14.xlsx")
    print("=" * 60)
