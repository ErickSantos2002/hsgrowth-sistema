...
# Executa: limpeza, migrations, inicialização e importação dos CSVs do Pipedrive
#

set -e  # Para na primeira falha

echo "================================================================================"
echo "🚀 HSGrowth CRM - Setup Completo para Produção (via CSV)"
echo "================================================================================"
echo ""
...
echo "  4. Importar dados dos CSVs do Pipedrive (se disponíveis)"
echo ""
...
# PASSO 4: Importar CSVs do Pipedrive (opcional)
echo ""
echo "================================================================================"
echo ""
echo -e "${BLUE}[4/4] Importação dos CSVs do Pipedrive${NC}"
echo ""

# Verifica se a pasta pipedrive existe
if [ ! -d "pipedrive" ]; then
    echo -e "${YELLOW}⚠️  Pasta 'pipedrive' não encontrada${NC}"
    echo ""
    echo "Para importar dados do Pipedrive:"
    echo "  1. Exporte os CSVs do Pipedrive"
    echo "  2. Crie a pasta: backend/pipedrive/"
    echo "  3. Coloque todos os CSVs nessa pasta"
    ...
    if ask_yes_no "Deseja pular a importação do Pipedrive?"; then
        ...
        echo ""
        echo -e "${RED}❌ Setup interrompido. Adicione os CSVs e execute novamente.${NC}"
        exit 1
    fi
else
    echo -e "${GREEN}✅ Pasta pipedrive encontrada${NC}"
    echo ""

    # Conta quantos CSVs tem na pasta
    csv_count=$(ls -1 pipedrive/*.csv 2>/dev/null | wc -l)

    if [ $csv_count -eq 0 ]; then
        echo -e "${YELLOW}⚠️  Nenhum arquivo CSV encontrado na pasta pipedrive${NC}"

        if ask_yes_no "Deseja pular a importação?"; then
            echo -e "${YELLOW}⏭️  Pulando importação do Pipedrive${NC}"
        else
            echo ""
            echo -e "${RED}❌ Setup interrompido.${NC}"
            exit 1
        fi
    else
        echo -e "${GREEN}✅ Encontrados $csv_count arquivos CSV${NC}"
        echo ""

        if ask_yes_no "Deseja importar dados dos CSVs agora?"; then
            echo ""
            $EXEC_PREFIX python scripts/import_from_pipedrive_csv.py << EOF
IMPORTAR
EOF

            if [ $? -eq 0 ]; then
                echo -e "${GREEN}✅ Importação dos CSVs concluída com sucesso${NC}"
            else
                echo -e "${RED}❌ Erro durante importação dos CSVs${NC}"
                exit 1
            fi
        else
