"""
Script para mover cards ganhos do Board 2 para Lista 17 do Board 5.

Execução:
    docker exec -it hsgrowth-api python scripts/move_won_cards.py
"""
import sys
import os
...
def move_won_cards_from_board2_to_board5():
    """Move todos os cards ganhos do Board 2 para a Lista 17 do Board 5."""
    ...
    try:
        ...
        print("=" * 60)
        print("SCRIPT: Mover Cards Ganhos")
        print("=" * 60)
        ...
        # Busca todos os cards ganhos do Board 2
        # is_won: 0=aberto, 1=ganho, -1=perdido
        won_cards = db.query(Card).filter(
            Card.list_id.in_(source_list_ids),
            Card.is_won == 1
        ).all()

        total_cards = len(won_cards)

        if total_cards == 0:
            print(f"\n✓ Nenhum card ganho encontrado no Board {SOURCE_BOARD_ID}")
            print("   Nada a fazer!")
            return

        print(f"\n📊 Encontrados {total_cards} cards ganhos no Board {SOURCE_BOARD_ID}")
        print("-" * 60)

        # Confirma antes de executar
        print("\n⚠️  ATENÇÃO: Esta operação irá mover TODOS os cards ganhos!")
...
if __name__ == "__main__":
    move_won_cards_from_board2_to_board5()
